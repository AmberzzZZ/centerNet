...
def centerNext(input_shape=(512,512,3), n_classes=80, n_stacks=2, n_channles=[256, 384, 384, 384, 512]):
...
def hourglass_module(inpt, n_classes, n_channles):
    ...
    for i in range(4):
        x = resnext(x, n_channles[-1], strides=1)
...
def InceptionDownSamp(x):
    ...
    x1 = MaxPooling2D(pool_size=3, strides=2, padding='same')(x)
